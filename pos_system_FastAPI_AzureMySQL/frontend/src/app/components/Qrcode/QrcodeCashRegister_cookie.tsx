...
export default function QrcodeReaderComponent({ userName }) {
    ...
    // useEffectを使用してサーバーサイドから受け取ったuserNameを利用する例
    useEffect(() => {
        console.log(`ようこそ ${userName} さん！`);
        // 他の必要な処理があればここに追加
    }, [userName]); // userNameが変更されたときに実行される
...
export const getServerSideProps = async (context:any) => {
    ...
    console.log(cookies); // サーバーサイドでクッキーの内容を確認
    const userName = cookies.user_name;

    // ユーザー名がクッキーに存在しない場合、ログインページにリダイレクト
    if (!userName) {
        return {
            redirect: {
                destination: '/login',
